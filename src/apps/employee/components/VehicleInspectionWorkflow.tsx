...
import { Car, CheckCircle, XCircle, Camera, Upload, ArrowRight, ArrowLeft } from 'lucide-react';
...
export const VehicleInspectionWorkflow: React.FC<VehicleInspectionWorkflowProps> = ({
  ...
  const [currentStep, setCurrentStep] = useState<'vehicle' | 'odometer' | 'checklist' | 'additional'>('vehicle');
  ...
  const validateChecklistStep = (): boolean => {
    const incompleteItems = formData.inspectionItems.filter(item => item.status === null);
    
    if (incompleteItems.length > 0) {
      setErrors({ checklist: `Please complete all ${incompleteItems.length} remaining items` });
      return false;
    }
  ...
  const handleNext = () => {
    if (currentStep === 'vehicle' && validateVehicleStep()) {
      setCurrentStep('odometer');
    } else if (currentStep === 'odometer' && validateOdometerStep()) {
      setCurrentStep('checklist');
    } else if (currentStep === 'checklist' && validateChecklistStep()) {
  ...
  const handleSubmit = async () => {
    if (!validateChecklistStep()) return;
  ...
  const getProgress = () => {
    switch (currentStep) {
      case 'vehicle': return 25;
      case 'odometer': return 50;
      case 'checklist': return 75;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            <div className="text-sm text-slate-500 mb-1">Step {
              currentStep === 'vehicle' ? '1' :
              currentStep === 'odometer' ? '2' :
              currentStep === 'checklist' ? '3' : '4'
            } of 4</div>
            <div className="text-lg font-semibold text-slate-900">{getStepTitle()}</div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        {currentStep === 'vehicle' && (
          <div className="space-y-6">
            <div className="flex items-center space-x-3 mb-6">
              ...
              <h2 className="text-xl font-semibold text-slate-900">Your Assigned Vehicle</h2>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="text-center">
                        ...
                        <div className="text-sm text-slate-500 mt-2">Vehicle assigned by employer</div>
                      </div>
                    </div>
                  </div>
                </label>
        ...
        {currentStep === 'odometer' && (
          <div className="space-y-6">
            ...
            <div className="flex space-x-3">
              ...
              >
                <span>Continue to Daily Check</span>
                <ArrowRight className="h-4 w-4" />
              </button>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            {errors.checklist && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <p className="text-red-800 text-sm">{errors.checklist}</p>
              </div>
            )}
                ...
                <div key={index} className="border border-slate-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex space-x-2">
                      <button
                        type="button"
                        onClick={() => updateInspectionItem(index, 'status', 'ok')}
                      ...
                      <button
                        type="button"
                        onClick={() => updateInspectionItem(index, 'status', 'defect')}
                        className={`flex items-center space-x-2 px-3 py-2 rounded-lg transition-colors ${
                          item.status === 'defect'
                        ...
                        <div className="flex items-center space-x-3">
                          <label className="flex items-center px-3 py-2 border border-red-300 rounded-lg hover:bg-red-50 cursor-pointer">
                            ...
                            <input
                              type="file"
                              accept="image/*"
                              onChange={(e) => updateInspectionItem(index, 'photo', e.target.files?.[0] || null)}
                              className="sr-only"
                            />
                          </label>
                          {item.photo && (
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg transition-colors flex items-center justify-center space-x-2"
              >
                <span>Continue to Additional Plant</span>
                <ArrowRight className="h-4 w-4" />
              </button>
            </div>
          </div>
        ...
        {currentStep === 'additional' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold text-slate-900">Additional Plant Requirements (Items 11+)</h2>
              <button
