...
import { supabase, MollsworthRate } from '../../../lib/supabase';

export const MollsworthRatesManagement: React.FC = () => {
  const [mollsworthRates, setMollsworthRates] = useState<MollsworthRate[]>([]);
  const [filteredRates, setFilteredRates] = useState<MollsworthRate[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [showAddForm, setShowAddForm] = useState(false);
  const [editingRate, setEditingRate] = useState<MollsworthRate | null>(null);
  ...
  const handleDeleteRate = async (rate: MollsworthRate) => {
  ...
  const startEdit = (rate: MollsworthRate) => {
    setEditingRate(rate);
    setEditForm({
  ...
  const excavationOptions = ['-', 'NO/EXC', 'O/EXC', 'DUCT'];
  const siteOptions = ['-', 'SITE', 'AGRI', 'U/MADE', 'UM/CW', 'FWAY', 'CWAY', 'SURFACED', 'SOFT', 'IN/TREN'];
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Mollsworth Rates Management</h1>
              <p className="text-slate-600">Manage Mollsworth work rates and pricing</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            <button
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-200">
          <h3 className="text-lg font-semibold text-slate-900">
            Mollsworth Rates ({filteredRates.length})
          </h3>
        </div>
